...
class Retriever:
    def __init__(self, git_link, model_name="krlvi/sentence-msmarco-bert-base-dot-v5-nlpl-code_search_net") -> None:
            ...
            for file in filenames:
                ...
                if file_extension in file_extension_mapping.keys():
                    try:
                        ...
                        splitter = CodeSplitter(language=file_extension_mapping[file_extension],
                                                chunk_lines=40,
    ...
    def load_db(self):
        store_path = os.path.join(self.base_save_path, f"{self.clone_path.split('/')[-1]}_{self.model_name.split('/')[-1]}")
        if os.path.exists(store_path):
            if self.vector_index is None:
                db = chromadb.PersistentClient(path=store_path)
                chroma_collection = db.get_or_create_collection("chroma_collection")
                vector_store = ChromaVectorStore(chroma_collection=chroma_collection)
                self.doc_store = SimpleDocumentStore().from_persist_path(store_path+"/docstore")
                self.vector_index = VectorStoreIndex.from_vector_store(
                    vector_store,
                    embed_model=self.model,
                )

                self.node_postprocessor = CustomNodePostProcessor(docstore=self.doc_store)
            ...
            self.vector_index = VectorStoreIndex(doc_chunks, storage_context=storage_context,
                                                 embed_model=self.model)
    ...
    def retrieve_results(self, query):
        ...
        for node in response.source_nodes:
            response_texts.append({"page_content": node.text,
